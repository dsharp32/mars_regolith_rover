// Decode 3 r/c signals using interrupts.
...
int current_sense; // variable to hold current sensor value

int current_limit = 5;  // sets the amperage limit that when exceeded on either motor, tells the motor driver to cut power to both motors for 1 second.

int cool_off = 10000; // number of milliseconds to stop motors if current limit is reached
...
void loop() {
  ...
  current_sense = current_sense + AcsValue;  //Add samples together
  delay (3); // let ADC settle before next sample 3ms
}

current_sense = current_sense/150.0;//Taking Average

  if (current_sense > current_limit){
    stop_scoop();
    left_stop();
    right_stop();
    delay(cool_off);
